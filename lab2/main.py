...
def embed_kjb(cover: QImage, bits: list[int], lam: float, seed: int):
    ...
    rng.shuffle(all_indices)  # Перемешиваем индексы пикселей
    used_indices = all_indices[:len(bits)]  # Выбираем пиксели для встраивания
    for i, bit_val in enumerate(bits):
        linear_index = used_indices[i]
        y = linear_index // width  # Вычисляем координаты пикселя
        x = linear_index % width
        color = result.pixelColor(x, y)
        R, G, B_val = color.red(), color.green(), color.blue()
        Y = brightness(R, G, B_val)  # Вычисляем яркость
        if bit_val == 1:
            B_new = B_val + lam * Y  # Увеличиваем синий канал
        else:
            B_new = B_val - lam * Y  # Уменьшаем синий канал
        B_new = max(0, min(255, B_new))  # Ограничиваем значение
        color.setBlue(int(B_new))
        result.setPixelColor(x, y, color)  # Обновляем пиксель
...
def measure_blue_diff(original: QImage, watermarked: QImage) -> float:
    if original.isNull() or watermarked.isNull():
        return 0.0
    width, height = original.width(), watermarked.height()
...
class KJBVisualizer(QMainWindow):
    def __init__(self):
        ...
        self.setStyleSheet("""
            ...
            QPlainTextEdit, QLineEdit, QDoubleSpinBox {
                border: 1px solid #ccc;
                border-radius: 4px;
                padding: 5px;
            }
    ...
    def init_embed_tab(self):
        ...
        self.txt_input.setStyleSheet("font-size: 14px;")
        control_layout.addWidget(self.txt_input)

        group_lambda = QGroupBox("λ")
        ...
        self.btn_embed.setObjectName("extractButton")
    ...
    def init_extract_tab(self):
        ...
        self.btn_extract.setObjectName("extractButton")
        ...
        self.btn_measure_error.setObjectName("saveButton")
